...
export default function ContractModal({ thread, offer, user, onClose, onSigned, onNegotiateInstead }) {
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      await api.post(`/chat/v2/threads/${thread.id}/sign`, { offer_id: offer?.id });
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          {/* Footer - Still Negotiate and Accept via OTP (as explicitly requested) */}
          <div className="p-5 border-t border-[var(--border-strong)] bg-[var(--bg-elevated)]/20 flex justify-between gap-3 shrink-0">
            
            <button 
              onClick={() => {
                onClose();
                if (onNegotiateInstead) {
                  onNegotiateInstead(offer);
                } else if (window.handleNegotiateInstead) {
                  window.handleNegotiateInstead(offer);
                }
              }}
