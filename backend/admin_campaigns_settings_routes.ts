...
export function setupAdminCampaignsSettingsRoutes(
  ...
  router.post("/admin/users/:user_id/ban", async (req, res) => {
    const user = await parseAuthUser(req);
    if (!user || (user.role !== 'admin' && user.role !== 'sub_admin' && user.team_role !== 'sub_admin')) {
      return res.status(403).json({ detail: "Admin only", _status: 403 });
    }
    ...
    const activeClient = privilegedSupabase || supabase;
    if (activeClient) {
      const { error } = await activeClient.from('users').update({ banned: true }).eq('user_id', req.params.user_id);
      if (error) {
        return res.status(500).json({ error: "Failed to ban user in Supabase: " + error.message });
      }
      await logAdminAction(user, 'ban_user', 'user', req.params.user_id, { reason: "Banned" });
    }
